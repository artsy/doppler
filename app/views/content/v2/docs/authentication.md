...
```
curl -v -X POST "#{ArtsyAPI::V2.root}/tokens/xapp_token?client_id=...&client_secret=..."
```
...
```
curl -v -X POST "#{ArtsyAPI::V2.url}/oauth2/access_token?client_id=...&client_secret=...&code=...&grant_type=authorization_code"
```
...
```
curl -v -X POST "#{ArtsyAPI::V2.url}/oauth2/access_token?client_id=...&client_secret=...&grant_type=credentials&email=...&password=..."
```
...
```
curl -v -X POST "#{ArtsyAPI::V2.url}/oauth2/access_token?client_id=...&client_secret=...&grant_type=oauth_token&oauth_token=...&oauth_provider=..."
```
...
```
curl -v -X DELETE "#{ArtsyAPI::V2.root}/tokens/access_token?access_token=..."
```
